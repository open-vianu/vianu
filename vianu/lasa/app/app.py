...
from vianu.lasa.settings import GRADIO_APP_NAME, GRADIO_RESULTS_COLUMNS, GRADIO_RESULTS_COLUMNS_DEFAULT
...
_UI_SETTINGS_SOURCES = [(name, value) for name, value in zip(['Swissmedic', 'FDA'], SOURCES)]
# NOTE: make sure that the following dicts is mapping names to the corresponding field names in :class:`Match`
_UI_SETTINGS_RESULTS_COLUMNS = [(name, value) for name, value in zip(GRADIO_RESULTS_COLUMNS, ['sounds_alike', 'looks_alike', 'combined'])]
...
class App(BaseApp):
        ...
        with gr.Column(scale=5):
            with gr.Row(elem_classes='search-container'):
                ...
                with gr.Column(scale=1, elem_classes='search-button'):
                    self._components['search.start'] = gr.HTML("<div class='search-start'>Search</div>", visible=True)
                    self._components['search.running'] = gr.HTML("<div class='search-running'>running...</div>", visible=False)
    ...
    # --------------------------------------------------------------------------
    @staticmethod
    def _toggle_button(session_state: SessionState) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    @staticmethod
    def _setup_session_state(session_state: SessionState, source: List[str], threshold: float, apply_threshold_to: str, search: str) -> SessionState:
    ...
    def _get_search_results(self, local_state: dict, session_state: SessionState) -> pd.DataFrame:
        # Get matches
        args_ = {
        ...
        matches = [m for m in matches if getattr(m, session_state.apply_threshold_to) >= normalized_threshold]
        ...
        logger.debug(f"dataframe has shape={value.shape} with threshold={session_state.threshold}")
    ...
    def _event_start_search(self):
        gr.on(
            triggers=[self._components['search.query'].submit, self._components['search.start'].click],
