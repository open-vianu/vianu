...
def process(lasa: LASA, names: Iterable[str], processes: int = PROCESSES) -> List[Match]:
    """Perform the LASA matches in parallel with a pool of processes"""
    logger.debug(f"matching ref={lasa._ref} against #names={len(names)} with #processes={PROCESSES}")
